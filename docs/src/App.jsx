            ...
            <div className="w-full px-2 overflow-x-hidden">
                <div className="w-full max-w-2xl bg-gray-800 p-6 rounded-lg shadow-lg overflow-x-auto">
                    ...
                    <div className="mt-4 flex justify-between">
                        <button onClick={handleSubmitAnswer} className={`px-4 py-2 rounded-md transition-transform duration-300 ${canSubmit ? "bg-green-500 text-white hover:bg-green-700 active:opacity-70" : "bg-gray-500 text-gray-300 cursor-not-allowed"}`} disabled={!canSubmit}>
                            Submit Answer
                        </button>
                    </div>
                    {showResult && (
                                    ...
                                    <div className="text-red-600">
                                        ...
                                        <div className="mt-2 text-sm">
                                            ...
                                            {currentQuestion.answer.map((answer, idx) => {
                                                ...
                                                return isMultiline ? (
                                                <div key={idx} className="font-mono bg-transparent text-black whitespace-pre-wrap break-words">
                                                    {renderFormattedText(answer)}
                                                </div>
                                                ) : (
                                                <div key={idx} className="flex items-start space-x-2">
                                                    <span className="mt-[0.25rem]">•</span>
                                                    <div>{renderFormattedText(answer)}</div>
                                                </div>
                                                );
                                            })}
                                            </div>
